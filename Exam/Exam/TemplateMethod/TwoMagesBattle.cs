...
namespace DP_Laba_2.TemplateMethod
{
    ...
    class TwoMagesBattle : WorkHard
    {
        protected override void CreateHeroes()
        {
            ...
            HB.SetTheHero(new MageBuilder(), names[0], (Guilds)allGuilds.GetValue(rn.Next(allGuilds.Length)), race[0]);
            HB.FillUnitsProperties();
            _heroes.Add(new Wizard(HB.GetUnit()));

            System.Threading.Thread.Sleep(5);

            HB.SetTheHero(new MageBuilder(), names[1], (Guilds)allGuilds.GetValue(rn.Next(allGuilds.Length)), race[1]);
            HB.FillUnitsProperties();
            _heroes.Add(new Wizard(HB.GetUnit()));
        ...
        protected override void Fight()
        {
            ...
            do
            {
                ...
                Console.WriteLine($"\n{_heroes[0].Name}`s health {_heroes[0].Health[0].ToString("N2")}\\{_heroes[0].Health[1]} mana {_heroes[0].Mana[0].ToString("N2")}\\{_heroes[0].Mana[1]}");

                Console.WriteLine($"{_heroes[1].Name}`s health {_heroes[1].Health[0].ToString("N2")}\\{_heroes[1].Health[1]} mana {_heroes[1].Mana[0].ToString("N2")}\\{_heroes[1].Mana[1]}");
