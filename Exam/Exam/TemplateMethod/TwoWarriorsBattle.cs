...
namespace DP_Laba_2.TemplateMethod
{
    using Builder;
    using Decorator;
    class TwoWarriorsBattle : WorkHard
    {
        protected override void CreateHeroes()
        {
            ...
            HB.SetTheHero(new WarriorBuilder(), names[0], (Guilds)allGuilds.GetValue(rn.Next(allGuilds.Length)), race[0]);
            HB.FillUnitsProperties();
            _heroes.Add(new Warrior(HB.GetUnit()));

            System.Threading.Thread.Sleep(5);

            HB.SetTheHero(new WarriorBuilder(), names[1], (Guilds)allGuilds.GetValue(rn.Next(allGuilds.Length)), race[1]);
            HB.FillUnitsProperties();
            _heroes.Add(new Warrior(HB.GetUnit()));
        ...
        protected override void Fight()
        {
            ...
            do
            {
                ...
                Console.WriteLine($"\n{_heroes[0].Name}`s health {_heroes[0].Health[0].ToString("N2")}\\{_heroes[0].Health[1]} stamina {_heroes[0].Stamina[0].ToString("N2")}\\{_heroes[0].Stamina[1]}");

                Console.WriteLine($"{_heroes[1].Name}`s health {_heroes[1].Health[0].ToString("N2")}\\{_heroes[1].Health[1]} stamina {_heroes[1].Stamina[0].ToString("N2")}\\{_heroes[1].Stamina[1]}");
